...
function Frame:render()
	...
	if props.AspectRatio then
		local Constraint = new('UIAspectRatioConstraint', { AspectRatio = props.AspectRatio })
		...
		if props.DominantAxis == 'Width' then
			...
			if typeof(props.Width) == 'UDim' then
				...
				props.Size = UDim2.new(props.Size.X, UDim.new(props.Size.X.Scale / props.AspectRatio, 0))
			end
		...
		elseif props.DominantAxis == 'Height' then
			...
			if typeof(props.Height) == 'UDim' then
				...
				props.Size = UDim2.new(UDim.new(props.Size.Y.Scale * props.AspectRatio, 0), props.Size.Y)
			end
		end
	end
	...
	if props.Layout == 'List' then
		...
		props[Roact.Children] = Support.Merge({ Layout = Layout }, props[Roact.Children])
	end
...
function Frame:GetSize(ContentSize)
	...
	local Padding = UDim2.new(0, props.HorizontalPadding or 0, 0, props.VerticalPadding or 0)
