...
local function makeSchema(name, fn)
    local check = {[IS_SCHEMA] = true, name = name, validate = fn}

    setmetatable(check, {__call = function(_, ...) return fn(...) end})
            ...
            if not success then
                local message =
                    ("Bad argument %s (#%d), expected %s, but %s"):format(
                        argName, index, argSchema.name, err)
                error(message, 3)
            end
...
function Typer.instance(expectedInstanceClass)
    assert(typeof(expectedInstanceClass) == "string",
           "expectedInstanceClass must be a string")
    ...
    return makeSchema(name, function(list)
        ...
        for key, value in pairs(list) do
            ...
            if keyType ~= "number" then
                return false, ("got non-number key %s (of type %s)"):format(
                           tostring(key), keyType)
            end
...
function Typer.mapOf(keyCheck, valueCheck)
    ...
    return makeSchema(name, function(map)
        ...
        if not mapOk then return false, mapErr end
        ...
        for key, value in pairs(map) do
            ...
            if not keyOk then
                return false,
                       ("had bad key %s: %s"):format(tostring(key), keyErr)
            end
            ...
            if not valueOk then
                return false, ("had bad value %s at key %s"):format(
                           tostring(value), tostring(key), valueErr)
            end
        end
